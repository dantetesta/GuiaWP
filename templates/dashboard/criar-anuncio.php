<?php
/**
 * Template: Criar Anúncio (Multi-Step Redesign)
 *
 * @package GuiaWP
 ...
 * @modified 1.9.7 - 2026-03-21 - 6 steps (contato/endereco separados), icones SVG de redes sociais, sidebar sticky single
 */
...
<main class="flex-1 lg:ml-64 min-h-screen flex flex-col min-w-0 overflow-x-hidden">
	...
	<header id="gcep-form-header" class="bg-white/95 backdrop-blur-md sticky top-14 lg:top-0 z-10 border-b border-slate-200">
		<div class="px-4 lg:px-8 flex items-center justify-between" style="height:56px">
			<div class="flex items-center gap-3 min-w-0">
				<a href="<?php echo esc_url( home_url( '/painel/anuncios' ) ); ?>" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors flex-shrink-0" aria-label="<?php esc_attr_e( 'Voltar para anúncios', 'guiawp' ); ?>">
					<span class="material-symbols-outlined text-xl">arrow_back</span>
				</a>
				<h2 class="text-sm font-bold text-slate-900 truncate"><?php esc_html_e( 'Criar Anúncio', 'guiawp' ); ?></h2>
			</div>
			<!-- Contador mobile -->
	...
	<div class="p-4 sm:p-6 lg:p-8 max-w-[900px] w-full mx-auto flex-1">
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data" id="gcep-multistep-form">
			<input type="hidden" name="action" value="gcep_save_anuncio">
			<input type="hidden" name="gcep_anuncio_id" id="gcep_anuncio_id" value="0">
			...
			<div data-step="1">
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200">
					<h2 class="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
						...
						<?php esc_html_e( 'Defina seu Anúncio', 'guiawp' ); ?>
					</h2>
					<p class="text-xs text-slate-500 mb-5"><?php esc_html_e( 'Escolha o tipo de anúncio e o plano desejado.', 'guiawp' ); ?></p>

					<div class="grid grid-cols-1 gap-5">
						<div>
							<span class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block"><?php esc_html_e( 'Tipo de Anúncio', 'guiawp' ); ?></span>
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
								<label class="relative cursor-pointer">
									<input type="radio" name="gcep_tipo_anuncio" value="empresa" checked class="peer sr-only">
								...
								<label class="relative cursor-pointer">
									<input type="radio" name="gcep_tipo_anuncio" value="profissional_liberal" class="peer sr-only">
									<div class="border-2 border-slate-200 peer-checked:border-[#0052cc] peer-checked:bg-[#0052cc]/5 rounded-xl p-4 transition-all flex items-center gap-3">
										<div class="w-10 h-10 rounded-lg bg-[#0052cc]/10 flex items-center justify-center flex-shrink-0">
										...
										<div>
											...
											<p class="text-[11px] text-slate-500 leading-tight mt-0.5"><?php esc_html_e( 'Para autônomos e freelancers.', 'guiawp' ); ?></p>
										</div>
									</div>
								</label>
							</div>
						...
						<div>
							...
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
								<label class="relative cursor-pointer">
									<input type="radio" name="gcep_tipo_plano" value="gratis" checked class="peer sr-only">
									<div class="border-2 border-slate-200 peer-checked:border-[#0052cc] peer-checked:bg-[#0052cc]/5 rounded-xl p-4 transition-all flex items-center gap-3">
										...
										<div>
											<h3 class="text-sm font-bold text-slate-900"><?php esc_html_e( 'Grátis', 'guiawp' ); ?></h3>
											<p class="text-[11px] text-slate-500 leading-tight mt-0.5"><?php esc_html_e( 'Informações básicas e contato.', 'guiawp' ); ?></p>
										</div>
									</div>
								</label>
								<label class="relative cursor-pointer">
									<input type="radio" name="gcep_tipo_plano" value="premium" class="peer sr-only">
									<div class="border-2 border-slate-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 rounded-xl p-4 transition-all flex items-center gap-3">
										...
										<div>
											...
											<p class="text-[11px] text-slate-500 leading-tight mt-0.5"><?php esc_html_e( 'Galeria, vídeos, descrição e selo.', 'guiawp' ); ?></p>
										</div>
									</div>
								</label>
							</div>
					...
					<div id="gcep-premium-plans" class="hidden mt-5">
						<input type="hidden" name="gcep_plano_id" id="gcep_plano_id" value="0">
						<span class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block"><?php esc_html_e( 'Escolha a Vigência', 'guiawp' ); ?> <span class="text-rose-500">*</span></span>
						...
						<div class="bg-amber-50 border border-amber-200 rounded-lg p-4 text-center">
							...
							<p class="text-xs text-amber-700 font-medium"><?php esc_html_e( 'Nenhum plano premium disponível no momento. Entre em contato com o administrador.', 'guiawp' ); ?></p>
						</div>
						<?php endif; ?>
					</div>
				</section>
				...
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200">
					...
					<div class="grid grid-cols-1 gap-4">
						<label class="flex flex-col gap-1.5">
							...
							<input type="text" name="gcep_titulo" required class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none transition-all" placeholder="<?php esc_attr_e( 'Ex: Agência Digital Pro', 'guiawp' ); ?>">
						</label>
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div data-show-tipo="empresa">
								<label class="flex flex-col gap-1.5">
									...
									<input type="text" name="gcep_cnpj" id="gcep_cnpj" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="00.000.000/0000-00" maxlength="18">
								</label>
							</div>

							<?php if ( ! empty( $categorias ) && ! is_wp_error( $categorias ) ) : ?>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Categoria', 'guiawp' ); ?> <span class="text-rose-500">*</span></span>
								<select name="gcep_categoria[]" required class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none">
						...
						<div data-gratis-only>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Descrição Curta', 'guiawp' ); ?></span>
								<textarea name="gcep_descricao_curta" rows="3" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="<?php esc_attr_e( 'Uma breve frase que resume seu serviço...', 'guiawp' ); ?>"></textarea>
							</label>
						</div>

						<!-- Descrição Longa (somente premium) -->
						<div data-premium-only class="hidden">
				...
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200">
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<label class="flex flex-col gap-1.5">
							...
							<input type="tel" name="gcep_telefone" data-intl-tel class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="(00) 9 9999-9999">
						</label>
						<label class="flex flex-col gap-1.5">
							<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'WhatsApp', 'guiawp' ); ?></span>
							<input type="tel" name="gcep_whatsapp" data-intl-tel class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="(00) 9 9999-9999">
						</label>
						<label class="flex flex-col gap-1.5">
							<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'E-mail', 'guiawp' ); ?></span>
							<input type="email" name="gcep_email" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="contato@empresa.com">
						</label>
						<label class="flex flex-col gap-1.5">
							<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Site', 'guiawp' ); ?></span>
							<input type="url" name="gcep_site" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="https://seusite.com.br">
						</label>
					</div>
				</section>
			</div>
					...
					<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<label class="flex flex-col gap-1.5">
								...
								<input type="text" name="gcep_cep" id="gcep_cep" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="00000-000" maxlength="9">
							...
							<label class="flex flex-col gap-1.5">
								...
								<input type="text" name="gcep_logradouro" id="gcep_logradouro" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="<?php esc_attr_e( 'Rua, Avenida...', 'guiawp' ); ?>">
							</label>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Número', 'guiawp' ); ?></span>
								<input type="text" name="gcep_numero" id="gcep_numero" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="123">
							</label>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Complemento', 'guiawp' ); ?></span>
								<input type="text" name="gcep_complemento" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="<?php esc_attr_e( 'Sala, Andar...', 'guiawp' ); ?>">
							</label>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Bairro', 'guiawp' ); ?></span>
								<input type="text" name="gcep_bairro" id="gcep_bairro" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none">
							</label>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Cidade', 'guiawp' ); ?></span>
								<input type="text" name="gcep_cidade" id="gcep_cidade" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" readonly>
							</label>
							<label class="flex flex-col gap-1.5">
								<span class="text-sm font-semibold text-slate-700"><?php esc_html_e( 'Estado (UF)', 'guiawp' ); ?></span>
								<input type="text" name="gcep_estado" id="gcep_estado" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" maxlength="2" readonly>
							</label>
						</div>
						...
						<div class="mt-4">
							...
							<p class="text-xs text-slate-400 mb-2"><?php esc_html_e( 'Após preencher o CEP, ajuste o pin para definir a posição exata.', 'guiawp' ); ?></p>
							<div id="gcep-address-map" class="hidden rounded-xl border border-slate-200 overflow-hidden z-0"></div>
							<input type="hidden" name="gcep_latitude" id="gcep_latitude" value="">
							<input type="hidden" name="gcep_longitude" id="gcep_longitude" value="">
						</div>
					</section>
			</div>
			...
			<div data-step="5" class="hidden">
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200">
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						$redes = [
							[ 'gcep_instagram', 'Instagram', 'https://instagram.com/...', 'instagram' ],
							[ 'gcep_facebook',  'Facebook',  'https://facebook.com/...',  'facebook' ],
							[ 'gcep_linkedin',  'LinkedIn',  'https://linkedin.com/...',  'linkedin' ],
							[ 'gcep_youtube',   'YouTube',   'https://youtube.com/...',   'youtube' ],
							[ 'gcep_x_twitter', 'X (Twitter)', 'https://x.com/...',       'x_twitter' ],
							[ 'gcep_tiktok',    'TikTok',    'https://tiktok.com/@...',   'tiktok' ],
							[ 'gcep_threads',   'Threads',   'https://threads.net/@...',  'threads' ],
						];
						...
						<label class="flex flex-col gap-1.5">
							...
							<input type="url" name="<?php echo esc_attr( $rede[0] ); ?>" class="rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-[#0052cc]/20 outline-none" placeholder="<?php echo esc_attr( $rede[2] ); ?>">
						</label>
						<?php endforeach; ?>
					</div>
			...
			<div data-step="6" class="hidden">
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
					...
					<!-- Logo / Foto Principal (crop 1:1 500x500) -->
					...
					<!-- Foto de Capa (crop 3.5:1 1400x400) -->
					<div class="gcep-crop-wrapper">
			...
			<div data-step="7" class="hidden">
				<section class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
					...
					<!-- Galeria de Fotos (somente premium) -->
					<div data-premium-only class="hidden" id="gcep-gallery-section">
						<span class="text-sm font-semibold text-slate-700 mb-2 block flex items-center gap-2">
							<?php esc_html_e( 'Galeria de Fotos', 'guiawp' ); ?>
							<span id="gcep-gallery-count" class="text-xs text-slate-400 ml-auto">0 / 20</span>
						</span>
						<p class="text-xs text-slate-400 mb-3"><?php esc_html_e( 'Até 20 fotos. JPG, PNG, GIF, WebP, AVIF ou HEIC. Máx 15 MB cada.', 'guiawp' ); ?></p>

						<div id="gcep-gallery-grid" class="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-3"></div>
					...
					<!-- Galeria de Vídeos (somente premium) -->
					<div data-premium-only class="hidden">
						<span class="text-sm font-semibold text-slate-700 mb-2 block flex items-center gap-2">
						...
						<div id="gcep-videos-container" class="space-y-3 mb-3"></div>
		...
		<!-- Overlay de resultado da validacao -->
		<div id="gcep-result-overlay" class="hidden">
			<!-- Carregando (durante submit AJAX) -->
			<div id="gcep-result-loading" class="hidden bg-white rounded-2xl border border-slate-200 shadow-sm p-8 sm:p-10 text-center">
				...
				<h3 class="text-lg font-bold text-slate-900 mb-1"><?php esc_html_e( 'Validando seu anuncio...', 'guiawp' ); ?></h3>
			...
			<div id="gcep-result-rejected" class="hidden bg-white rounded-2xl border border-rose-200 shadow-sm p-8 sm:p-10">
				<div class="text-center mb-6">
					...
					<h3 class="text-xl font-bold text-slate-900 mb-1"><?php esc_html_e( 'Anúncio não aprovado', 'guiawp' ); ?></h3>
				...
				<div class="flex flex-col sm:flex-row gap-3 justify-center">
					...
					<a href="<?php echo esc_url( home_url( '/painel/anuncios' ) ); ?>" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 text-slate-700 font-semibold hover:bg-slate-100 transition-all text-sm">
	...
	<div id="gcep-nav-bottom" class="sticky bottom-0 bg-white/95 backdrop-blur-md border-t border-slate-200 px-4 sm:px-6 z-20" style="padding-top:12px;padding-bottom:max(12px,env(safe-area-inset-bottom))">
		<div class="max-w-[900px] mx-auto flex justify-between items-center">
			...
			<button type="button" id="gcep-step-next" class="px-6 py-2.5 rounded-xl bg-[#0052cc] hover:bg-[#003d99] text-white font-bold shadow-lg shadow-[#0052cc]/20 transition-all inline-flex items-center gap-2 text-sm">
				<?php esc_html_e( 'Próximo', 'guiawp' ); ?>
				<span class="material-symbols-outlined text-base">arrow_forward</span>
			</button>

			<button type="button" id="gcep-step-submit" style="display:none" class="px-6 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold shadow-lg shadow-emerald-500/20 transition-all items-center gap-2 text-sm">
				<span class="material-symbols-outlined text-base">check_circle</span>
				<?php esc_html_e( 'Validar e Enviar', 'guiawp' ); ?>
			</button>
		</div>
	</div>
	...
	(function () {
		...
		var fallbackReason = <?php echo wp_json_encode( __( 'A validação automática reprovou o anúncio, mas não retornou uma justificativa detalhada. Revise título, descrição e dados de contato antes de tentar novamente.', 'guiawp' ) ); ?>;
		...
		function ensureReasonBox() {
			...
			if (buttons) {
				rejected.insertBefore(box, buttons);
			} else {
				rejected.appendChild(box);
			}
		...
		new MutationObserver(syncRejectedState).observe(rejected, {
			attributes: true,
